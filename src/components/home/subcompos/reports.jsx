                ...
                !Cookies.get('userData') ? (
                    ...
                    <>
                        ...
                        <div className='col-span-1 md:col-span-2 flex flex-col gap-4'>
                            ...
                            {datas?.incomes?.length > 0 || datas?.expenses?.length > 0 ? (
                                <div className='w-full h-auto bg-white shadow rounded'>
                                    ...
                                    <div className='w-full h-auto bg-white shadow grid grid-cols-1 lg:grid-cols-2 place-items-center'>
                                        {datas?.incomes?.length > 0 ? (
                                            <div className={`w-full flex justify-center items-center p-4 ${datas?.expenses?.length === 0 ? "col-span-1 lg:col-span-2" : ""}`}>
                                                <IncomeExpensePieChart type={"income"} data={datas} className={"w-full max-w-sm"} currency={userData?.currency_type} />
                                            </div>
                                        ) : null}
                                        {datas?.expenses?.length > 0 ? (
                                            <div className={`${datas?.incomes?.length === 0 ? "col-span-1 lg:col-span-2" : ""} w-full flex justify-center items-center p-4`}>
                                ...
                                <div className='w-full bg-white shadow rounded'>
                                    <div className='relative'>
                                        ...
                                        <div className='flex gap-1 absolute md:top-2 top-1.5 right-2'>
                                            <button onClick={() => setIncomeExpenseReportType(IncomeExpenseReportType === "monthly" ? "yearly" : "monthly")} className='bg-[#4495E4] hover:bg-[#317cbd] text-white px-3 text-sm md:text-md py-1 rounded' type="button">
                                                {IncomeExpenseReportType === "monthly" ? "Yearly" : "Monthly"}
                                            </button>
